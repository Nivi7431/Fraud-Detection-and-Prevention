...
class DocumentSystemApp:
    def __init__(self, root):
        ...
        self.fade_time = 500  # Duration of fade animation in milliseconds

        self.show_page("login")

    def show_page(self, page_name):
        if self.current_page:
            self.fade_out(self.current_page)
        self.current_page = self.pages[page_name]
        self.fade_in(self.current_page)

    def fade_in(self, widget):
        widget.update()
        for alpha in range(0, 100):
            widget.config(bg=f'#{alpha:02x}{alpha:02x}{alpha:02x}')
            widget.update_idletasks()
            widget.after(self.fade_time // 100)
